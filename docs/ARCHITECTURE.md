...
### 媒体资产安全与访问控制
- **Owner 级别资产登记**：用户上传图片时，自动在媒体资产表 (`media_assets`) 中登记文件与所有者归属；
- **三层穿透鉴权**：请求 `/uploads/<filename>` 时执行多主体穿透检查（上传所有者、引用该图片的足迹拥有者、以及处于配对状态的情侣伴侣），拒绝非法越权下载；
- **Token 职责分离**：主登录 JWT 禁止出现在 URL 查询参数中；媒体访问强制使用受限只读的 `media` scope 短期 Token，防止浏览器历史记录与代理访问日志泄露系统管理凭据。

### 离线 Outbox 队列与本地覆盖层 (Local Mutation Overlay)
- **前端双向同步韧性**：离线创建、编辑或删除足迹自动压入 IndexedDB Outbox 队列；
- **Local Mutation Overlay 保护**：重新连接云端拉取全量足迹时，系统自动先叠加应用本地尚未同步完成的 Outbox 变动，确保离线编辑绝对不被云端旧状态静默抹去；
- **可视化异常队列介入**：状态角标自动汇总失败与待传项，支持用户按需查看、单项重试、全部重试或手动丢弃。

### 多端 Revision 乐观并发锁 (OCC)
- 足迹记录内嵌自增版本号 `revision`；
- 更新接口严格比对客户端携带的 `revision` 与服务器端当前版本。若发生多设备并发修改，后提交端返回 `409 Conflict`，避免静默覆盖。

### 机密凭据物理隔离与全量灾备
- **密钥物理分离**：地图 API Key、多云存储 Secret 与数据库连接串独立写入受 `0o600` 权限保护的 `runtime_secrets.json`，普通偏好写入 `runtime_config.json`；
- **全量灾难恢复 ZIP**：支持一键打包与一键还原，涵盖全部记录、扩展特性配置以及本地存储的实体媒体照片。
